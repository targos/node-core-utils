...
function backportMain(options) {
  ...
  switch (status.step) {
    ...
    case steps.APPLY:
    case steps.CONFLICT:
      return continueToApply(options);
    default:
...
function startSession(options) {
  ...
  if (!options.sha) {
    throw new Error('Please provide at least on SHA to backport');
  }
...
function continueToApply(options) {
  if (!options.continue) {
    throw new Error(
  ...
  todo.push(commitBackport());
  const tasks = new Listr(todo, getListrOptions(options));
  return tasks.run(options);
}
...
function applyPatches() {
  return {
    ...
    task: async(ctx) => {
      ...
      let patchIndex = patches.findIndex((sha) => sha === currentPatch);
      if (status.step === steps.CONFLICT) {
        patchIndex++;
      }
